...
window.addEventListener('DOMContentLoaded', () => {
    ...
    const SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbxedz0NzpPg5yt_2hHGdkbGZMblTQoSjxpTGTwn7Ay0wCgvR_ELHZ1pe7ox6gpho4Ib/exec';
    ...
    async function updatePermitStatus(permitId, newStatus) {
        if (SCRIPT_URL.includes('GANTI_DENGAN_URL')) {
            showMessage('URL Google Apps Script belum dikonfigurasi. Silakan perbarui variabel SCRIPT_URL di ptw.js.');
            return;
        }
    ...
    async function createPermit(permitData) {
        ...
        try {
            ...
            if (result.success) {
                ...
                throw new Error(result.message || 'Terjadi kesalahan yang tidak diketahui saat membuat izin.');
            }
    ...
    async function fetchSheet(id, sheet, maxRetries = 3) {
        ...
        for (let attempt = 0; attempt < maxRetries; attempt++) {
            try {
                ...
                if (res.status === 400) throw new Error(`Gagal mengambil data (Error 400). Pastikan Google Sheet Anda sudah dibagikan dengan "Anyone with the link".`);
                ...
                try {
                    localStorage.setItem(`ptw_offline_${sheet}`, JSON.stringify({ data, timestamp }));
                } catch (e) {
                    console.warn("Gagal menyimpan data offline:", e);
                }
                console.log(`Data berhasil diambil dari jaringan untuk sheet: ${sheet}`);
    ...
    // 2. If network fails, try to load from localStorage
        ...
        showMessage(errorMessage);
        return { data: [], timestamp: null, isOffline: false, error: { message: errorMessage, sheetId: id } };
    }
    ...
    function applyFiltersAndSort(permits) {
        ...
        const filtered = permits.filter(p => {
            ...
            const statusMatch = currentFilters.status === 'all' || (p.status && p.status.toLowerCase() === currentFilters.status.toLowerCase());
    ...
    async function renderPermitList() {
        ...
        const detailWrapper = contentArea.querySelector('.ptw-detail-container-wrapper');
        ...
        const { data: permitsData, timestamp: lastUpdateTimestamp, isOffline, error } = await fetchSheet(id, sheet);
        ...
        if (statusFilter && statusFilter.options.length === 0) {
            ...
            statusFilter.innerHTML = statusOptions.map(opt => `<option value="${opt.toLowerCase()}">${opt}</option>`).join('');
        ...
        const tableHeadersHTML = `<tr>${Object.keys(headersConfig).map(key => {
        ...
        // Render baris tabel
        const tableRowsHTML = processedPermits.length === 0
            ? `<tr><td colspan="${Object.keys(headersConfig).length + 1}" class="table-body-cell text-center">Tidak ada izin kerja yang cocok dengan filter Anda.</td></tr>`
            : processedPermits.map(p => `
                <tr class="table-body-row">
                    ${Object.keys(headersConfig).map(key => {
                        let cellContent = p[key] || '';
                        if (key === 'status') {
                            return `<td class="table-body-cell"><span class="badge ${getStatusClass(cellContent)}">${cellContent}</span></td>`;
                        }
                        return `<td class="table-body-cell">${cellContent}</td>`;
                    }).join('')}
                    <td class="table-body-cell text-right">
                        <button class="table-action-button" data-id='${p.id}'>Lihat Detail</button>
                    </td>
                </tr>
            `).join('');
        ...
        if (offlineNoticeContainer) {
            ...
            <div class="offline-notice">
                ...
                <strong>${new Date(lastUpdateTimestamp).toLocaleString('id-ID', { dateStyle: 'full', timeStyle: 'short' })}</strong>
            </div>
            ` : '';
        }
    ...
    function renderPermitDetail(permit) {
        ...
        const existingDetail = contentArea.querySelector('.ptw-detail-container-wrapper');
        ...
        const statusDropdownHtml = statusOptions.map(opt =>
            `<option value="${opt}" ${permit.status === opt ? 'selected' : ''}>${opt}</option>`
        ).join('');
        if (currentMap) {
            currentMap.remove();
            currentMap = null;
        ...
        detailWrapper.innerHTML = `
            <div class="ptw-detail-container max-w-4xl mx-auto w-full">
                ...
                <h2 class="text-3xl font-bold text-gray-800 mb-2">${permit.title}</h2>
                <p class="text-sm text-gray-500 mb-6">ID Izin: ${permit.id} | Kategori: ${permit.category || 'N/A'}</p>

                <div class="grid md:grid-cols-2 gap-6 mb-6">
                    <div class="space-y-4">
                        <div>
                            <h3 class="font-semibold text-gray-700">Status Saat Ini</h3>
                            <span class="mt-1 badge ${getStatusClass(permit.status)}">${permit.status}</span>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <p class="text-gray-600">${permit.startDate} s/d ${permit.endDate}</p>
                        </div>
                        ...
                        <div class="bg-gray-50 border border-gray-200 p-4 rounded-lg">
                            ...
                            <div class="flex items-center space-x-3">
                                ...
                                <button id="update-status-btn" data-id="${permit.id}" class="button-primary">
        ...
        const locationParts = (permit.location || "").split(',').map(part => parseFloat(part.trim()));
        if (locationParts.length === 2 && !isNaN(locationParts[0]) && !isNaN(locationParts[1])) {
            ...
            if (mapContainer) {
                ...
                L.marker([lat, lon]).addTo(currentMap).bindPopup(`<b>${permit.title}</b>`).openPopup();
                
                setTimeout(() => currentMap.invalidateSize(), 100);
            }
        } else {
            document.getElementById('map-container').innerHTML = '<p class="text-gray-500 text-center p-8">Format lokasi tidak valid untuk peta.</p>';
        }
        lucide.createIcons();
    }
    ...
    if (createPermitBtn) {
        createPermitBtn.addEventListener('click', (e) => {
            ...
            if (createPermitModal) {
                ...
                console.error('Elemen modal "create-permit-modal" tidak ditemukan di HTML.');
    ...
    const closeModalBtnForCreate = document.getElementById('close-create-modal-btn');
